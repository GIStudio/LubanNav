...
朝向来源优先级：/heading（GPS）> /imu > /odom。
静止时 NMEA 常不发 /heading，此时会回退到 odom/imu（odom 为相对航向，绝对导航精度有限）。
"""
import math
import os
...
class UEBridgeNode(Node):
    def __init__(self):
        ...
        self.lat      = None
        self.lon      = None
        # 分源保存朝向；读取时按 heading > imu > odom 优先级选用。
        self._yaw_heading = None
        self._yaw_imu = None
        self._yaw_odom = None
        self._yaw_fallback_logged = False
        ...
        self.get_logger().info(
            f"朝向订阅：{HEADING_TOPIC}（优先） / {IMU_TOPIC} / {ODOM_TOPIC}（回退）"
        )
    ...
    def _on_heading(self, msg: QuaternionStamped):
        with self._lock:
            self._yaw_heading = quaternion_to_yaw(msg.quaternion)

    def _on_imu(self, msg: Imu):
        with self._lock:
            self._yaw_imu = quaternion_to_yaw(msg.orientation)

    def _on_odom(self, msg: Odometry):
        with self._lock:
            self._yaw_odom = quaternion_to_yaw(msg.pose.pose.orientation)

    def _get_yaw(self):
        """返回 (yaw_rad, source) 或 (None, None)。"""
        with self._lock:
            if self._yaw_heading is not None:
                return self._yaw_heading, "heading"
            if self._yaw_imu is not None:
                return self._yaw_imu, "imu"
            if self._yaw_odom is not None:
                return self._yaw_odom, "odom"
            return None, None
    ...
    def _handle_navigate(self, dest: dict, speed_str: str, cmd_id: str, robot_id: str):
        ...
        cur_yaw, yaw_src = self._get_yaw()
        if cur_yaw is None:
            self.get_logger().error(
                "朝向未就绪，无法导航（/heading、/imu、/odom 都没有可用 yaw）"
            )
            return
        if yaw_src != "heading" and not self._yaw_fallback_logged:
            self._yaw_fallback_logged = True
            self.get_logger().warn(
                f" /heading 无数据，导航临时使用 {yaw_src} 朝向；"
                "odom 为相对航向，绝对指北精度有限"
            )

        self.get_logger().info(
            f"导航目标({coord_source})：lat={target_lat:.8f}, lon={target_lon:.8f}  "
            f"速度：{linear_spd:.2f} m/s  朝向源：{yaw_src}"
        )
        self._stop_direction_motion()
        self._cancel_nav()
    ...
    def _nav_loop(self, target_lat, target_lon, linear_spd, cmd_id, robot_id):
        ...
        while not self._nav_stop.is_set():
            with self._lock:
                cur_lat = self.lat
                cur_lon = self.lon
            cur_yaw, _yaw_src = self._get_yaw()
