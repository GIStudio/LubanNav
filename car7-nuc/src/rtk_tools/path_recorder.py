...
class PathRecorderNode(Node):
    def __init__(self, auto_start=False):
        ...
        self.last_lat = None
        self.last_lon = None
        self.last_fix = None         # (lat, lon, alt) 最新有效 fix，供手动打点
    ...
    def mark_waypoint(self):
        ...
        self.waypoints.append((lat, lon, alt, time.time()))
        self._publish_path()
        e, n, _ = gps_to_enu(lat, lon, alt, *self.origin)
        print(
            f"\n[打点] #{len(self.waypoints)}  Lat:{lat:.7f} Lon:{lon:.7f}  "
            f"ENU:({e:+.2f},{n:+.2f})m"
        )
