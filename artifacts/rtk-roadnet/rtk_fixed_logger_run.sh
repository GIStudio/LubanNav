...
if command -v ros2 >/dev/null 2>&1 && [ -f "${ROS_SETUP:-}" ]; then
    exec python3 src/rtk_tools/rtk_fixed_logger.py
fi

RTK_ROADNET_CONTAINER="${RTK_ROADNET_CONTAINER:-campuscar-stm32-hoverboard}"
if [ -n "${RTK_ROADNET_CONTAINER:-}" ] && docker ps --format '{{.Names}}' 2>/dev/null | grep -qx "${RTK_ROADNET_CONTAINER}"; then
    # ROS 栈在容器内；挂载卷 /workspace/campusCar-new-chassis == /home/pc/campusCar
    exec docker exec "${RTK_ROADNET_CONTAINER}" bash -c \
